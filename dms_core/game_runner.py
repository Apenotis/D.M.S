...
def launch_game(map_data: tuple) -> None:
        ...
        if item == "1":
            ...
            if target_path:
                ...
                if item.lower() not in ["doom", "heretic", "hexen"]:
                    if os.path.isdir(os.path.join(cfg.BASE_DIR, "mods", sub_folder, item)):
    ...
    if mod_flag and cfg.USE_MODS and engine_name in MOD_COMPATIBLE_ENGINES:
        ...
        if len(mod_options) > 1:
            ...
            choice = input(f"\n  {Colors.YELLOW}Wahl (ENTER = Keine): {Colors.WHITE}").strip()
            if choice.isdigit() and 0 < int(choice) < len(mod_options):
                ...
                if selected_mod.startswith("[AUTO] "):
                    mod_folder_path = os.path.join(cfg.BASE_DIR, "mods", selected_mod.replace("[AUTO] ", ""))
                else:
                ...
                if os.path.exists(mod_folder_path):
                    for f in os.listdir(mod_folder_path):
                        if f.lower().endswith(VALID_EXTS):
                            mod_params.extend(["-file", os.path.join(mod_folder_path, f)])

    # ========================================================================
    # 3. BEFEHL ZUSAMMENBAUEN
    # ========================================================================
    iwad_full_path = os.path.join(cfg.IWAD_DIR, core)
    cmd = [engine_path, "-iwad", iwad_full_path] + file_params + mod_params + extra_params
    ...
    if cfg.DEBUG_MODE:
        ...
        inner_w = 80
        print(f"\n  {Colors.RED}╭{'─' * inner_w}╮")
        print(f"  │ {Colors.WHITE}{'SYSTEM-CHECK VOR SPIELSTART (DEBUG)':^{inner_w-2}} {Colors.RED}│")
        ...
        print(f"  {Colors.GRAY}ENGINE:  {Colors.BLUE}{cfg.CURRENT_ENGINE}{Colors.WHITE} ({os.path.basename(engine_path)})")
        ...
        if found_files_list:
            ...
            print(f"  {Colors.RED}  [!] KEINE ZUSATZDATEIEN (PWADS) IM ORDNER GEFUNDEN!")
        ...
        choice = input(f"  {Colors.GREEN}ENTER zum Starten{Colors.WHITE} / {Colors.RED}0 zum Abbruch: {Colors.WHITE}").strip()
