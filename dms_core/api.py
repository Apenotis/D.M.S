...
def get_installed_pwads():
    """Scannt die Ordner nach bereits installierten Dateien."""
    installed = []
...
def fetch_folder_files(folder_name):
    """Holt Datei-Listen von der Doomworld API."""
...
def download_idgames(file_data):
    """Lädt ein ausgewähltes WAD herunter und installiert es."""
    ...
    if "heretic" in api_dir:
        core_wad, category = "heretic.wad", "EXTRA"
    elif "hexen" in api_dir:
        core_wad, category = "hexen.wad", "EXTRA"
    ...
    try:
        ...
        print(
            f"  {Colors.GREEN}[OK]{Colors.WHITE} Als {Colors.YELLOW}{new_id}{Colors.WHITE} registriert."
        )
        time.sleep(2)
...
def search_doomworld():
    """Haupteinstieg für die Doomworld-Suche."""
    try:
        utils.clear_screen()
        print(
        ...
        choice = input(
            f"\n  {Colors.YELLOW}WAHL (ENTER zum Abbrechen): {Colors.WHITE}"
        ).strip()
        if not choice:
            return

        all_results = _perform_api_search(choice)
        if not all_results:
            print(f"  {Colors.RED}Nichts gefunden!{Colors.WHITE}")
            time.sleep(2)
            return

        all_results.sort(key=lambda x: float(x.get("rating", 0) or 0), reverse=True)
        _display_and_handle_results(all_results)
...
def _perform_api_search(choice):
    """Führt die eigentliche API-Abfrage durch."""
    all_results = []
    if choice == "1":
        ...
        paths = {
            "2": ["levels/doom/megawads/"],
            "3": ["levels/doom2/megawads/"],
            "4": ["levels/heretic/"],
            "5": ["levels/hexen/"],
        }
...
def _display_and_handle_results(all_results):
    """Zeigt die Ergebnisse an und verarbeitet Nutzereingaben (Paginierung)."""
    idx = 0
    page = 20
    while True:
        utils.clear_screen()
        max_idx = min(idx + page, len(all_results))
        print(
            f"\n  {Colors.MAGENTA}--- ERGEBNISSE ({idx + 1} bis {max_idx}) ---{Colors.WHITE}"
        )
        print(
            f"  {Colors.GRAY}{'ID':<4} {'Titel':<50} {'Größe':<12} {'Rating':<15} {'Status'}{Colors.WHITE}"
        )
        print(f"  {'-' * 110}")
        ...
        for i in range(idx, max_idx):
            ...
            fname = r.get("filename", "").lower().split("/")[-1]
            is_ins = fname in installed or os.path.splitext(fname)[0] in str(installed)

            col = Colors.GREEN if is_ins else Colors.WHITE
            status = f"{Colors.GREEN}[INSTALL]{Colors.WHITE}" if is_ins else ""

            print(
                f"  {Colors.YELLOW}{i + 1:<4}{col} {title:<50} {Colors.CYAN}{size:<12} {Colors.YELLOW}{stars:<15} {status}"
            )

        cmd = (
            input(
                f"\n  {Colors.YELLOW}[Nr] Install  [N] Weiter  [B] Zurück  [ENTER] Ende: {Colors.WHITE}"
            )
            .lower()
            .strip()
        )

        if cmd == "n" and idx + page < len(all_results):
            idx += page
        elif cmd == "b" and idx > 0:
            idx -= page
        elif cmd.isdigit():
            n = int(cmd) - 1
            if 0 <= n < len(all_results):
                download_idgames(all_results[n])
                break
        else:
            break
