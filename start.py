...
def get_w(c, min_w=25):
    ...
    for i in c:
        if i and i[0] != "EMPTY":
            raw = str(i[0]).replace("\n", "").replace("\r", "")
            clean = raw.split(" - ", 1)[-1].replace("__L__", "").replace("[L]", "").strip()
...
def format_entry_clean(item, width, l_id, name_color, is_col4=False):
    if not item or item[0] == "EMPTY": return " " * width
    ...
    styled = clean.replace("[C]", f"{Colors.CYAN}[C]{n_col}").replace("[M]", f"{Colors.RED}[M]{n_col}")
...
def main():
    ...
    try:
        ...
        if time.time() > next_check:
            ...
            config["UPDATE"]["next_check"] = str(time.time() + 604800) # 604800 Sek = 7 Tage
    ...
    while True:
        ...
        mod_count = sum(len([d for d in os.listdir(os.path.join(cfg.BASE_DIR, "mods", s)) if os.path.isdir(os.path.join(cfg.BASE_DIR, "mods", s, d))]) for s in ["doom", "heretic", "hexen"] if os.path.isdir(os.path.join(cfg.BASE_DIR, "mods", s)))
        ...
        upd_icon = f" {Colors.MAGENTA}[U]{Colors.WHITE}" if (uz_upd and cfg.CURRENT_ENGINE == "uzdoom") else ""
        m_on = "ON" if cfg.USE_MODS else "OFF"; s_on = "ON" if cfg.SHOW_STATS else "OFF"; d_on = "ON" if cfg.DEBUG_MODE else "OFF"
        ...
        w1, w2, w3, w4 = get_w(col1, 23), get_w(col2, 23), get_w(col3, 23), get_w(col4, 25)
        ...
        for i in range(max_rows):
            r1 = format_entry_clean(col1[i] if i<len(col1) else None, w1, last_id, Colors.RED)
            r2 = format_entry_clean(col2[i] if i<len(col2) else None, w2, last_id, Colors.GREEN)
            r3 = format_entry_clean(col3[i] if i<len(col3) else None, w3, last_id, Colors.GREEN)
            r4 = format_entry_clean(col4[i] if i<len(col4) else None, w4, last_id, Colors.WHITE, True)
        ...
        # --- BEFEHLE & EINGABE ---
        cmds = [
            f"{Colors.YELLOW}[0]{Colors.WHITE} Beenden", f"{Colors.YELLOW}[?]{Colors.WHITE} Zufall",
            f"{Colors.YELLOW}[R]{Colors.WHITE} Reset", f"{Colors.YELLOW}[C]{Colors.WHITE} Installer",
            f"{Colors.YELLOW}[D]{Colors.WHITE} DoomWorld", f"{Colors.YELLOW}[ID]c{Colors.WHITE} Clear",
            f"{Colors.YELLOW}[ID]m{Colors.WHITE} Skip", f"{Colors.YELLOW}[ID]x{Colors.WHITE} Delete",
        ...
        choice = input(f"    {Colors.YELLOW}ID ODER ENTER ({Colors.MAGENTA}{last_id}{Colors.YELLOW}): {Colors.WHITE}").strip().lower()

        # --- VERARBEITUNG DER EINGABE ---
        if choice == "0": sys.exit(0)
        elif choice == "e": engines.select_engine(); db.save_settings(); continue
        elif choice == "c": installer.run_installer(); continue
        elif choice == "d": api.search_doomworld(); continue
        elif choice == "r": os.execv(sys.executable, [sys.executable] + sys.argv)
        elif choice == "/m": cfg.USE_MODS = not cfg.USE_MODS; db.save_settings(); continue
        elif choice == "/s": cfg.SHOW_STATS = not cfg.SHOW_STATS; db.save_settings(); continue
        elif choice == "/d": cfg.DEBUG_MODE = not cfg.DEBUG_MODE; db.save_settings(); continue
        elif choice == "?":
            all_m = [i for b in blocks.values() for i in b if i and i[0] != "EMPTY"]
            if all_m: runner.launch_game(random.choice(all_m))
            continue
        
        if choice == "" and last_id: choice = last_id.lower()
        if len(choice) > 1 and choice[-1] in "xcm":
            tid = choice[:-1].upper()
            if choice.endswith("m"): db.toggle_mod_skip(tid)
            elif choice.endswith("c"): db.toggle_map_clear(tid)
            elif choice.endswith("x"): db.uninstall_map(tid)
            continue
        
        found = None
        for b in blocks.values():
            for it in b:
                if it and str(it[1]).lower() == choice: found = it; break
        if found: runner.launch_game(found)
        else: last_error = choice
